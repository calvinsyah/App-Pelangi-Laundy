...
function renderFormLinenInput() {
  ...
  const cardList = document.getElementById("linenCardList"); if (cardList) cardList.innerHTML = "";
  if (!pelId) { tbody.innerHTML = '<tr><td colspan="4">Pilih pelanggan terlebih dahulu.</td></tr>'; if (cardList) cardList.innerHTML = '<p style="text-align:center;color:var(--text-light);padding:24px;">Pilih pelanggan terlebih dahulu.</p>'; return; }
  ...
  if (linenList.length === 0) {
    ...
    if (cardList) cardList.innerHTML = '<p style="text-align:center;color:var(--text-light);padding:24px;">Belum ada linen yang diatur untuk jenis nota ini.</p>';
    return;
  }
    ...
    if (cardList) {
      cardList.innerHTML += `
        <div class="linen-card-item">
          ...
          <input type="number" class="input-qty linen-item-qty" data-id="${item.id}" data-name="${item.name}" data-price="${hargaSatuan}" value="0" min="0">
        </div>`;
    }
  });
}
...
function onEditJenisChange() {
  ...
  // Update card-list prices
  const cardPrices = container.querySelectorAll(".linen-card-item .price");
  cardPrices.forEach((priceEl, idx) => {
    const mid = parseInt(priceEl.closest('.linen-card-item').querySelector('.modal-edit-qty').getAttribute("data-masterid"));
    if (pData) { const newPrice = getHargaPerPelanggan(pData.id, mid, mult); priceEl.innerText = `@ ${fmtRp(newPrice)}`; }
  });
...
async function bukaModalEditLinen(id) {
  ...
  if (pData.type === "RS") {
    ...
    // Add table-wrap and card-list for mobile
    let tableHtml = '<div class="linen-table-wrap"><table class="linen-table"><thead><tr><th>Nama Linen</th><th style="text-align:center">Jumlah</th><th>Harga</th></tr></thead><tbody>';
    let cardHtml = '<div class="linen-card-list">';
    linenList.forEach((entry) => {
      ...
      tableHtml += `<tr><td><strong>${m.name}</strong></td><td style="text-align:center"><input type="number" class="modal-edit-qty" data-masterid="${m.id}" value="${qty}" min="0"></td><td>@ ${fmtRp(hargaSatuan)}</td></tr>`;
      cardHtml += `
        <div class="linen-card-item">
          <div class="info">
          ...
          <input type="number" class="modal-edit-qty" data-masterid="${m.id}" value="${qty}" min="0" style="width: 70px;">
        </div>`;
    });
...
function switchCategory(cat) {
  ...
  sub.innerHTML = `<span class="group-label">${info.label}:</span>` + info.tabs.map((t) => `<button class="tab-btn" role="tab" aria-selected="false" aria-controls="${t[0]}" onclick="switchTab('${t[0]}')">${t[1]}</button>`).join("");
...
async function switchTab(tabId) {
  ...
  const tabBtn = document.querySelector(`[onclick="switchTab('${tabId}')"]`); if (tabBtn) {
    tabBtn.classList.add("active");
    tabBtn.setAttribute("aria-selected", "true");
  }
  // Update category active state based on current tab
  const catInfo = Object.entries(TAB_CATEGORIES).find(([_, v]) => v.tabs.some(t => t[0] === tabId));
  if (catInfo) {
    document.querySelectorAll(".cat-btn").forEach((b) => {
...
function fabAction(action) {
  ...
  switch (action) {
    case 'invoice':
      ...
      setTimeout(() => switchTab('tab-invoice'), 100);
      break;
    case 'gaji':
...
document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape') {
    const fab = document.getElementById('fabBtn');
    const menu = document.getElementById('fabMenu');
    if (fab && menu && fab.classList.contains('open')) {
      fab.classList.remove('open');
      menu.classList.remove('open');
      fab.setAttribute('aria-expanded', 'false');
    }
...
/* ===== ACCESSIBILITY: Update aria-selected on tab switch ===== */
...
window.switchTab = async function(tabId) {
  ...
  document.querySelectorAll('.cat-btn').forEach(btn => {
    const onclick = btn.getAttribute('onclick') || '';
    const isActive = onclick.includes(tabId.replace('tab-', '')) || 
                     (tabId.startsWith('tab-') && TAB_CATEGORIES[btn.dataset.cat]?.tabs.some(t => t[0] === tabId));
    btn.setAttribute('aria-selected', isActive);
    btn.classList.toggle('active', isActive);
  });
};
