-- 20260713000016_update_dashboard_rpc_adm_kategori.sql
-- Memperbarui RPC get_dashboard_metrics agar kategori pengeluaran custom (lain-lain)
-- secara otomatis masuk ke perhitungan Administrasi & Umum (ADM).
...
BEGIN
    ...
    tagihan_per_bulan AS (
        SELECT
            ...
            COALESCE((
                ...
                WHERE ps.pelanggan_id = nc.pelanggan_id AND ps.bulan = nc.bulan
            ), false) AS is_paid
    ...
    SELECT json_build_object(
        ...
        'omset_lunas', COALESCE((SELECT sum(tagihan) FROM tagihan_final WHERE bulan = p_periode AND is_paid = true), 0),
        ...
        'adm', COALESCE((SELECT sum(nominal) FROM biaya_calc WHERE bulan = p_periode AND kategori NOT IN ('GAS','AIR','LISTRIK 1','LISTRIK 2','CHEMICAL','BBM','PLASTIK','PPH PS 23','GAJI BORONGAN')), 0),
        'laba', COALESCE((SELECT sum(tagihan) FROM tagihan_final WHERE bulan = p_periode AND is_paid = true), 0)
                ...
                - COALESCE((SELECT sum(nominal) FROM biaya_calc WHERE bulan = p_periode AND kategori NOT IN ('GAS','AIR','LISTRIK 1','LISTRIK 2','CHEMICAL','BBM','PLASTIK','PPH PS 23','GAJI BORONGAN')), 0),
        
        'kas', COALESCE((SELECT sum(tagihan) FROM tagihan_final WHERE is_paid = true AND bulan <= p_periode), 0)
               - COALESCE((SELECT sum(nominal) FROM biaya_calc WHERE lunas = true AND bulan <= p_periode), 0),
        'piutang', COALESCE((SELECT sum(tagihan) FROM tagihan_final WHERE is_paid = false AND bulan <= p_periode), 0),
        'utang', COALESCE((SELECT sum(nominal) FROM biaya_calc WHERE lunas = false AND bulan <= p_periode), 0)
                 + COALESCE((SELECT sum(active_utang) FROM utang_pinjaman), 0),
        'peralatan', COALESCE((SELECT peralatan FROM pengaturan_calc), 0),
        'modal', (COALESCE((SELECT sum(tagihan) FROM tagihan_final WHERE is_paid = true AND bulan <= p_periode), 0) - COALESCE((SELECT sum(nominal) FROM biaya_calc WHERE lunas = true AND bulan <= p_periode), 0)) -- kas
                 + COALESCE((SELECT sum(tagihan) FROM tagihan_final WHERE is_paid = false AND bulan <= p_periode), 0) -- piutang
                 + COALESCE((SELECT peralatan FROM pengaturan_calc), 0) -- peralatan
                 - (COALESCE((SELECT sum(nominal) FROM biaya_calc WHERE lunas = false AND bulan <= p_periode), 0) + COALESCE((SELECT sum(active_utang) FROM utang_pinjaman), 0)) -- utang
    ) INTO result;
